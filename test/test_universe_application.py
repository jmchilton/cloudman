...
def test_default_instance_types():
    with userdata({}):
        ...
        micro_group = instance_types[1][2]
        assert micro_group[0] == ("t1.micro", "Micro (t1.micro)", "Cores: Up to 2 EC2 Compute Units, Memory: 613MB, IO-Perf: Low, EBS-Optimized: No")
...
def test_nectar_instance_types():
    with userdata({"cloud_name": "nectar"}):
        app = UniverseApplication()
        instance_types = app.config.instance_types
        assert len(instance_types) == 6
        assert instance_types[5][0] == "m1.xxlarge"
